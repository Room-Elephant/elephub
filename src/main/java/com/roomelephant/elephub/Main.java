...
public class Main {
  ...
  public static void main(String[] args) {
    ...
    if (hasInvalidPath(socketPath)) {
      log.error("operation='main', message='Docker socket not found at  {}'", DOCKER_SOCKET);
      System.exit(1);
    }

    if (hasInvalidPermissions(socketPath)) {
      log.error(
          "operation='main', message='Insufficient permissions to access Docker socket at {}', "
              + "user='{}', permissions='{}'",
  ...
  private static DockerClient getDockerClient() {
    DockerClientConfig config = DefaultDockerClientConfig.createDefaultConfigBuilder()
        .withDockerHost("unix://" + DOCKER_SOCKET).withDockerTlsVerify(false).build();
